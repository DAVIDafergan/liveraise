...
const LiveScreen: React.FC = () => {
  ...
  // הגדרת מידות המסך לפי הניהול - שומר על איכות 4K
  const screenStyles: any = {
    ...
    width: campaign.displaySettings?.width ? `${campaign.displaySettings.width}px` : '100%',
  ...
  return (
    <div 
      className="text-white overflow-hidden flex flex-col font-sans" 
      dir="rtl" 
      style={screenStyles}
    >
      ...
      <div className={`flex-1 max-w-[98%] mx-auto w-full p-4 flex flex-col gap-4 min-h-0 ${!campaign.bannerUrl ? 'pt-24' : ''}`}>
        ...
        <div className="bg-white/5 p-4 rounded-[1.5rem] border border-white/10 shadow-2xl backdrop-blur-sm relative overflow-hidden">
          ...
          <div className="flex justify-between mt-2 text-sm font-bold opacity-70 px-1 tracking-widest">
            ...
            <div className="flex items-center gap-2"><Target size={16} /><span>יעד: {campaign.targetAmount.toLocaleString()} {campaign.currency}</span></div>
          </div>
        </div>
            ...
            <div className="scroll-marquee">
              <div className="grid grid-cols-5 gap-4 py-4">
                {allDonations.map((donation: any, idx: number) => (
                  <motion.div key={idx} className="bg-white/5 backdrop-blur-lg p-5 rounded-[2rem] border border-white/10 flex flex-col justify-between shadow-2xl" style={{ borderTop: `4px solid ${campaign.themeColor}` }}>
                    <div className="flex flex-col gap-2">
                      <span className="text-xl font-black text-white/90 truncate">{donation.fullName}</span>
                      {donation.dedication && <p className="text-sm italic opacity-60 line-clamp-2 h-10">"{donation.dedication}"</p>}
                    </div>
                    <div className="mt-4 pt-4 border-t border-white/5 text-2xl font-black" style={{ color: campaign.themeColor }}>₪{donation.amount.toLocaleString()}</div>
                  </motion.div>
                ))}
              </div>
            </div>
